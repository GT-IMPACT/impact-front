...
It has three operating modes. Modes are switched with a screwdriver. Can work with multiple forms (schemes) that are in different bases in mode \Buses separated on\ (switched with a screwdriver while holding down the Shift key). Recipe is executed with the shape (circuit) that is in the same bass as the material
